...
CREATE TABLE secondhouselog_login(
  device_id STRING,
  context_id STRING,
  city STRING,
  login_account STRING,
  ...
  passport_uid DOUBLE,
  phone STRING
)partitioned by (data_date string) COMMENT 'This is second house log login table'
STORED AS PARQUET;
...
insert into user_track.secondhouselog partition(data_date='2019-06-10') select device_id,context_id,city,login_account,start_time,end_time,content,object_id,context,secondhouse_id,esta_x,district_x,address_x,streetid_x,blockid,blockshowname,purpose,structure,buildtype,buildyear,buildarea,subfloor,floor,totalfloor,room,hall,toilet,kitchen,balcony,forward,price,averprice_x,environment,traffic,fitment,serverco,contactor,telno,mobile,creattime,updatetime,expiretime,city_name,block_id,blockname,district_y,streetid_y,area,address_y,bus,averprice_y,updateprice,forumid,newhouseid,b_map_x,b_map_y,accuracy,map_test,b_property_type,b_green,b_parking,b_developers,b_property_company,b_property_fees,b_bus,b_metro,b_num,bi_s,bi_spell,subway,sitename,subwayrange,app,esta_y,property_fees,nofee,plot_ratio,total_room,turn_time,b_area,feature from user_track.secondhouselog_csv
